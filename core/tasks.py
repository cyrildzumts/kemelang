...
def send_mail_task(email_context=None):
    if email_context is not None and isinstance(email_context, dict):
        try:
            ...
            logger.warning(f"Error when sending Email - Backend : {settings.EMAIL_BACKEND} : EMAIL_USER : {settings.EMAIL_HOST_USER}, EMAIL_HOST : {settings.EMAIL_HOST}, EMAIL_PORT : {settings.EMAIL_PORT} , CERT : {settings.EMAIL_SSL_CERTFILE}", e)
